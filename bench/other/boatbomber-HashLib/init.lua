...
sha2_H_ext512_lo, sha2_H_ext512_hi = {
	[384] = {},
	[512] = sha2_H_lo,
}, {
	[384] = {},
...
md5_K, md5_sha1_H = {}, { 0x67452301, 0xEFCDAB89, 0x98BADCFE, 0x10325476, 0xC3D2E1F0 }
md5_next_shift = {
	0,
	0,
	0,
...
function sha256_feed_64(H, str, offs, size)
	...
	for pos = offs, offs + size - 1, 64 do
		...
		for j = 1, 64 do
			z = bit32_bxor(bit32_rrotate(e, 6), bit32_rrotate(e, 11), bit32_lrotate(e, 7))
				+ bit32_band(e, f)
				+ bit32_band(-1 - e, g)
				+ h
				+ K[j]
...
function sha512_feed_128(H_lo, H_hi, str, offs, size)
	...
	for pos = offs, offs + size - 1, 128 do
		...
		for jj = 34, 160, 2 do
			a_lo, a_hi, b_lo, b_hi = W[jj - 30], W[jj - 31], W[jj - 4], W[jj - 5]
			tmp1 = bit32_bxor(
		...
		a_lo, b_lo, c_lo, d_lo, e_lo, f_lo, g_lo, h_lo = h1_lo, h2_lo, h3_lo, h4_lo, h5_lo, h6_lo, h7_lo, h8_lo
		a_hi, b_hi, c_hi, d_hi, e_hi, f_hi, g_hi, h_hi = h1_hi, h2_hi, h3_hi, h4_hi, h5_hi, h6_hi, h7_hi, h8_hi
		for j = 1, 80 do
			jj = 2 * j
			tmp1 = bit32_bxor(
...
function md5_feed_64(H, str, offs, size)
	...
	for pos = offs, offs + size - 1, 64 do
		...
		s = 25
		for j = 1, 16 do
			F = bit32_rrotate(bit32_band(b, c) + bit32_band(-1 - b, d) + a + K[j] + W[j], s) + b
			s = md5_next_shift[s]
			a = d
			d = c
			c = b
		...
		s = 27
		for j = 17, 32 do
			F = bit32_rrotate(bit32_band(d, b) + bit32_band(-1 - d, c) + a + K[j] + W[(5 * j - 4) % 16 + 1], s)
				+ b
			s = md5_next_shift[s]
			a = d
			d = c
		...
		s = 28
		for j = 33, 48 do
			F = bit32_rrotate(bit32_bxor(bit32_bxor(b, c), d) + a + K[j] + W[(3 * j + 2) % 16 + 1], s) + b
			s = md5_next_shift[s]
			a = d
			d = c
			c = b
		...
		s = 26
		for j = 49, 64 do
			F = bit32_rrotate(bit32_bxor(c, bit32_bor(b, -1 - d)) + a + K[j] + W[(j * 7 - 7) % 16 + 1], s) + b
			s = md5_next_shift[s]
			a = d
			d = c
			c = b
...
function keccak_feed(lanes_lo, lanes_hi, str, offs, size, block_size_in_bytes)
	...
	for pos = offs, offs + size - 1, block_size_in_bytes do
		for j = 1, qwords_qty do
			a, b, c, d = string.byte(str, pos + 1, pos + 4)
		...
		for round_idx = 1, 24 do
			C1_lo = bit32_bxor(L01_lo, L06_lo, L11_lo, L16_lo, L21_lo)
			C1_hi = bit32_bxor(L01_hi, L06_hi, L11_hi, L16_hi, L21_hi)
			C2_lo = bit32_bxor(L02_lo, L07_lo, L12_lo, L17_lo, L22_lo)
			C2_hi = bit32_bxor(L02_hi, L07_hi, L12_hi, L17_hi, L22_hi)
			C3_lo = bit32_bxor(L03_lo, L08_lo, L13_lo, L18_lo, L23_lo)
			C3_hi = bit32_bxor(L03_hi, L08_hi, L13_hi, L18_hi, L23_hi)
			C4_lo = bit32_bxor(L04_lo, L09_lo, L14_lo, L19_lo, L24_lo)
			C4_hi = bit32_bxor(L04_hi, L09_hi, L14_hi, L19_hi, L24_hi)
			C5_lo = bit32_bxor(L05_lo, L10_lo, L15_lo, L20_lo, L25_lo)
			C5_hi = bit32_bxor(L05_hi, L10_hi, L15_hi, L20_hi, L25_hi)

			D_lo = bit32_bxor(C1_lo, C3_lo * 2 + (C3_hi % TWO_POW_32 - C3_hi % TWO_POW_31) / TWO_POW_31)
			D_hi = bit32_bxor(C1_hi, C3_hi * 2 + (C3_lo % TWO_POW_32 - C3_lo % TWO_POW_31) / TWO_POW_31)

			T0_lo = bit32_bxor(D_lo, L02_lo)
			T0_hi = bit32_bxor(D_hi, L02_hi)
			T1_lo = bit32_bxor(D_lo, L07_lo)
			T1_hi = bit32_bxor(D_hi, L07_hi)
			T2_lo = bit32_bxor(D_lo, L12_lo)
			T2_hi = bit32_bxor(D_hi, L12_hi)
			T3_lo = bit32_bxor(D_lo, L17_lo)
			T3_hi = bit32_bxor(D_hi, L17_hi)
			T4_lo = bit32_bxor(D_lo, L22_lo)
			T4_hi = bit32_bxor(D_hi, L22_hi)
...
do
	...
	idx, step, p, one, sqrt_hi, sqrt_lo = 0, { 4, 1, 2, -2, 2 }, 4, { 1 }, sha2_H_hi, sha2_H_lo
	repeat
		p = p + step[p % 6]
		d = 1
		repeat
			...
			if d * d > p then
				...
				_, delta = mul(R, mul(R, R, 1, 4), -1, 4)
				hi = R[2] % 65536 * 65536 + math.floor(R[1] / 256)
				lo = R[1] % 256 * 16777216 + math.floor(delta * (TWO_POW_NEG_56 / 3) * root / p)
				...
				if idx < 16 then
					...
					_, delta = mul(R, R, -1, 2)
					hi = R[2] % 65536 * 65536 + math.floor(R[1] / 256)
					lo = R[1] % 256 * 16777216 + math.floor(delta * TWO_POW_NEG_17 / root)
					idx = idx % 8 + 1
	...
	function partial(message_part)
		if message_part then
			...
			if tail then
				...
				size = partLength - offs
				size_tail = size % block_size_in_bytes
			...
			if tail then
				...
				function get_next_part_of_digest(bytes_needed)
					...
					if bytes_needed <= last_part_size then
						...
						part_size_in_nibbles = bytes_needed * 2
						result = string.sub(last_part, 1, part_size_in_nibbles)
