...
function test()
	...
	function murmur3_x64_128(buf, seedh, seedl)
		len = buffer.len(buf)
		nblocks = len // 16
		...
		for i = 0, nblocks - 1 do
			off = i * 16
			k1h, k1l = buffer.readu32(buf, off + 4), buffer.readu32(buf, off)
			k2h, k2l = buffer.readu32(buf, off + 12), buffer.readu32(buf, off + 8)
		...
		if rem > 0 then
			...
			if rem >= 9 then
				...
				for j = 8, rem - 1 do
					shift = (j - 8) * 8
					byte = buffer.readu8(buf, tailStart + j)
					if shift < 32 then
