    ...
    for i = 0, 255 do
        ...
        for _ = 1, 8 do
            if bit32.band(lo, 1) == 1 then
                newlo = bit32.bxor(bit32.rshift(lo, 1), bit32.lshift(hi, 31))
                newhi = bit32.rshift(hi, 1)
...
function crc32_buf(buf: buffer, offset: number, len: number): number
    crc = 0xFFFFFFFF
    for i = 0, len - 1 do
        byte = buffer.readu8(buf, offset + i)
        crc = bit32.bxor(bit32.rshift(crc, 8), CRC32_TABLE[bit32.band(bit32.bxor(crc, byte), 0xFF)])
    end
    return bit32.bxor(crc, 0xFFFFFFFF)
end
...
function crc64(data: string): (number, number)
    ...
    for i = 1, data.count do
        ...
        newlo = bit32.bxor(bit32.bxor(bit32.rshift(lo, 8), bit32.lshift(hi, 24)), entry[2])
        newhi = bit32.bxor(bit32.rshift(hi, 8), entry[1])
        lo = newlo
        hi = newhi
    end
...
function crc64_bytes(data: string): string
    hi, lo = crc64(data)
...
function lzma_state_update_literal(state: number): number
    if state < 4 then return 0
    else if state < 10 then return state - 3
    else return state - 6 end
end

function lzma_state_update_match(state: number): number
    if state < 7 then return 7 else return 10 end
end

function lzma_state_update_rep(state: number): number
    if state < 7 then return 8 else return 11 end
end

function lzma_state_update_short_rep(state: number): number
...
function lzma2_decode(input: string, input_offset: number, expected_uncompressed_size: number, dict_size: number): string
    ...
    function reset_lzma_state()
        ...
        num_lit_probs = 768 * bit32.lshift(1, lc + lp)
    ...
    function dict_put_byte(byte: number)
    ...
    function decode_bit(probs, idx): number
        normalize()
        prob = probs[idx]
        bound = bit32.rshift(rc_range, 11) * prob
    ...
    function decode_bit_tree(probs, num_bits): number
        m = 1
        for _ = 1, num_bits do
    ...
    function decode_bit_tree_reverse(probs, offset, num_bits): number
        m = 1
        result = 0
        for i = 0, num_bits - 1 do
            b = decode_bit(probs, offset + m)
            m = m * 2 + b
    ...
    function decode_len(choice, choice2, low, mid, high, ps): number
    ...
    while true do
        ...
        if control == 1 or control == 2 then
            ...
            unpack_size = bit32.lshift(string.byte(input, pos), 8) + string.byte(input, pos + 1) + 1
            pos = pos + 2
            ...
            unpack_high = bit32.band(control, 0x1F)
            do_reset_dict = control >= 0xE0
            do_reset_state = control >= 0xA0
            do_new_props = control >= 0xC0

            unpack_size = bit32.lshift(unpack_high, 16) + bit32.lshift(string.byte(input, pos), 8) + string.byte(input, pos + 1) + 1
            pos = pos + 2
            pack_size = bit32.lshift(string.byte(input, pos), 8) + string.byte(input, pos + 1) + 1
            pos = pos + 2

            if do_new_props then
                props_byte = string.byte(input, pos)
                pos = pos + 1
                lc = props_byte % 9
                rem = math.floor(props_byte / 9)
                lp = rem % 5
                pb = math.floor(rem / 5)
                pos_mask = bit32.lshift(1, pb) - 1
                lit_pos_mask = bit32.lshift(1, lp) - 1
            ...
            while output_pos < unpack_size do
                ...
                sidx = state * LZMA_NUM_POS_STATES_MAX + cur_pos_state

                is_match_bit = decode_bit(is_match, sidx + 1)
                if is_match_bit == 0 then
                    ...
                    if state >= LZMA_NUM_LIT_STATES then
                        ...
                        symbol = 1
                        repeat
                            mb = bit32.band(bit32.rshift(match_byte, 7), 1)
                            match_byte = bit32.band(bit32.lshift(match_byte, 1), 0xFF)
                            b = decode_bit(lit_probs, po + 256 * (1 + mb) + symbol + 1)
                            symbol = symbol * 2 + b
                    ...
                    if decode_bit(is_rep, state + 1) == 0 then
                        ...
                        psv = decode_bit_tree(pos_slot_probs[ls], LZMA_NUM_POS_SLOT_BITS)

                        if psv >= 4 then
                            ndb = bit32.rshift(psv, 1) - 1
                            dist = bit32.lshift(bit32.band(psv, 1) + 2, ndb)

                            if psv < LZMA_END_POS_MODEL_INDEX then
                                dist = dist + decode_bit_tree_reverse(pos_special, dist - psv, ndb)
                            else
                                nf = ndb - LZMA_NUM_ALIGN_BITS
                                fv = 0
                                for j = nf - 1, 0, -1 do
                                    normalize()
                                    rc_range = bit32.rshift(rc_range, 1)
                                    rc_code = rc_code - rc_range
                                    t = -bit32.rshift(rc_code, 31)
                                    bv = bit32.band(bit32.bxor(t, -1), 1)
                    ...
                    actual_len = len + LZMA_MATCH_LEN_MIN
                    remaining = unpack_size - output_pos
...
function read_multibyte(data: string, pos: number): (number, number)
    val = 0
    shift = 0
    while true do
...
function xz_decompress(input: string): string
    ...
    while true do
        ...
        block_header_size = (header_size_byte + 1) * 4
        ...
        for _, filter in ipairs(filters) do
            if filter.id == 0x21 then  # LZMA2
                ...
                if prop_byte >= 40 then
                    ...
                    mantissa = bit32.band(prop_byte, 1) + 2
                    exponent = bit32.rshift(prop_byte, 1) + 11
...
function rc_encode_bit(rc, probs: { number }, idx: number, bit: number)
    prob = probs[idx]
    bound = bit32.rshift(rc.range, 11) * prob
    if bit == 0 then
        rc.range = bound
...
function rc_encode_bit_tree(rc, probs: { number }, num_bits: number, val: number)
    m = 1
    for i = num_bits - 1, 0, -1 do
        b = bit32.band(bit32.rshift(val, i), 1)
        rc_encode_bit(rc, probs, m + 1, b)
        m = m * 2 + b
    end
end

function rc_encode_bit_tree_reverse(rc, probs: { number }, offset: number, num_bits: number, val: number)
    m = 1
    for i = 0, num_bits - 1 do
        b = bit32.band(bit32.rshift(val, i), 1)
...
function rc_encode_direct_bits(rc, val: number, num_bits: number)
    for i = num_bits - 1, 0, -1 do
...
function rc_flush(rc)
    for _ = 1, 5 do
        rc_shift_low(rc)
    end
end

function rc_get_output(rc): string
...
function hash3(buf: buffer, pos: number): number
    b0 = buffer.readu8(buf, pos)
    b1 = buffer.readu8(buf, pos + 1)
    b2 = buffer.readu8(buf, pos + 2)
    return bit32.band(bit32.bxor(b0 * 256 + b1, b2 * 2654435761), HASH_MASK)
end

function hash4(buf: buffer, pos: number): number
    v = buffer.readu32(buf, pos)
...
function lzma_encode(data_buf: buffer, start_offset: number, end_offset: number, lc: number, lp: number, pb: number, dict_size: number, enc_state: any): (string, any)
    ...
    function encode_literal(pos: number, byte: number, prev_byte: number)
        ...
        if state >= LZMA_NUM_LIT_STATES then
            match_byte = buffer.readu8(data_buf, pos - rep0 - 1)
            symbol = 1
            context = byte
            for i = 7, 0, -1 do
                match_bit = bit32.band(bit32.rshift(match_byte, i), 1)
                cur_bit = bit32.band(bit32.rshift(context, i), 1)
                prob_idx = probs_offset + 256 * (1 + match_bit) + symbol + 1
            ...
            symbol = 1
            for i = 7, 0, -1 do
                cur_bit = bit32.band(bit32.rshift(byte, i), 1)
    ...
    function encode_distance(dist: number, len: number)
        ...
        if pos_slot_val >= 4 then
            num_direct_bits = bit32.rshift(pos_slot_val, 1) - 1
            base = bit32.lshift(bit32.band(pos_slot_val, 1) + 2, num_direct_bits)
            dist_reduced = dist - base

            if pos_slot_val < LZMA_END_POS_MODEL_INDEX then
                offset = base - pos_slot_val
    ...
    function find_matches_at(pos: number, r0: number, r1: number, r2: number, r3: number): { { number } }
        ...
        for ri, rep_dist in ipairs({ r0, r1, r2, r3 }) do
            if rep_dist < pos and rep_dist < dict_size then
                len = 0
                base = pos - rep_dist - 1
                rep_max = math.min(max_len, buffer.len(data_buf) - base)
        ...
        if pos + 3 < end_offset then
            ...
            if best_rep_len < NICE_LEN then
                ...
                while cur >= 0 and chain_count < MAX_CHAIN_LEN do
                    ...
                    len = 0
                    cur_max = math.min(max_len, buffer.len(data_buf) - cur)
    ...
    function distance_price(dist, len_raw)
        ...
        total = bit_tree_price(pos_slot_enc[len_state], LZMA_NUM_POS_SLOT_BITS, pos_slot_val)
    ...
    opt_dist  = table.create(OPT_WINDOW + MATCH_LEN_MAX + 2, 0)
    opt_state = table.create(OPT_WINDOW + MATCH_LEN_MAX + 2, 0)
    opt_r0    = table.create(OPT_WINDOW + MATCH_LEN_MAX + 2, 0)
    opt_r1    = table.create(OPT_WINDOW + MATCH_LEN_MAX + 2, 0)
    opt_r2    = table.create(OPT_WINDOW + MATCH_LEN_MAX + 2, 0)
    opt_r3    = table.create(OPT_WINDOW + MATCH_LEN_MAX + 2, 0)

    function literal_price(pos: number, cur_st: number, cur_r0: number, cur_r1: number, cur_r2: number, cur_r3: number)
        ps = bit32.band(pos, pos_mask)
        sidx = cur_st * LZMA_NUM_POS_STATES_MAX + ps
        cost = bit_price(is_match, sidx + 1, 0)
        ...
        lit_st = bit32.lshift(bit32.band(pos, lit_pos_mask), lc_val) + bit32.rshift(prev_byte, 8 - lc_val)
        po = lit_st * 768

        if cur_st >= LZMA_NUM_LIT_STATES and cur_r0 < pos then
            match_byte = buffer.readu8(data_buf, pos - cur_r0 - 1)
            symbol = 1
            for i = 7, 0, -1 do
                match_bit = bit32.band(bit32.rshift(match_byte, i), 1)
                cur_bit = bit32.band(bit32.rshift(cur_byte, i), 1)
                prob_idx = po + 256 * (1 + match_bit) + symbol + 1
                cost = cost + bit_price(lit_probs, prob_idx, cur_bit)
                symbol = symbol * 2 + cur_bit
                if match_bit != cur_bit then
                    for j = i - 1, 0, -1 do
                        b = bit32.band(bit32.rshift(cur_byte, j), 1)
            ...
            symbol = 1
            for i = 7, 0, -1 do
                b = bit32.band(bit32.rshift(cur_byte, i), 1)
                cost = cost + bit_price(lit_probs, po + symbol + 1, b)
                symbol = symbol * 2 + b
            end
        end
    ...
    function emit_token(pos: number, token_len: number, token_dist_or_rep: number, is_rep_match: boolean, is_short_rep: boolean)
        ...
        if not is_rep_match and token_len == 1 and token_dist_or_rep == 0 then
            ...
            if rep_idx == 1 then
                ...
                tmp = rep2; rep2 = rep1; rep1 = rep0; rep0 = tmp
    ...
    while pos < end_offset do
        ...
        for i = 1, win_size do
            ...
            if abs_pos >= cur_r0 + 1 and abs_pos < end_offset then
                if buffer.readu8(data_buf, abs_pos) == buffer.readu8(data_buf, abs_pos - cur_r0 - 1) then
                    ...
                    sr_cost = cur_cost + rep_match_price(cur_st, ps, 0, 0)
                    ...
                    if mdist < 0 then
                        ...
                        for l = MATCH_LEN_MIN, mlen do
                            ...
                            if mc < opt_cost[ti] then
                                ...
                                opt_dist[ti] = 0
                                nr0, nr1, nr2, nr3 = reps_after_rep(rep_idx, cur_r0, cur_r1, cur_r2, cur_r3)
                        ...
                        for l = MATCH_LEN_MIN, mlen do
                            ...
                            if mc < opt_cost[ti] then
                                ...
                                opt_dist[ti] = mdist
                                nr0, nr1, nr2, nr3 = reps_after_match(mdist, cur_r0, cur_r1, cur_r2, cur_r3)
        ...
        for ti = n_toks, 1, -1 do
            ...
            tok_type = tok_seq_type[ti]
            tok_dist = tok_seq_dist[ti]
    ...
    while offset < total_len do
        ...
        if pack_size > 65536 or pack_size + 6 >= unpack_size then
            ...
            while remaining > 0 do
                sub_size = math.min(remaining, 65536)
                ctrl = if first_chunk and sub_off == offset then 1 else 2
...
function xz_compress(input_data: string): string
    ...
    full_block_header = string.char(header_size_byte) .. block_header_content .. string.rep("\x00", header_padding)
    block_header_crc = crc32(full_block_header)
    ...
    footer_crc = crc32(footer_content)
