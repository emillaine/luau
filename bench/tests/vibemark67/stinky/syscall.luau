...
brkAddr = Int.ZERO
mmapBase = Int.fromHex("7000000000") # high address for mmap allocations
nextTid = 1000
outputLine = print
...
function Syscall.init(highAddr: integer, output: (string) -> ())
    ...
    pageSize = Int.from(4096)
    aligned = Int.band(Int.add(highAddr, Int.sub(pageSize, Int.ONE)), Int.bnot(Int.sub(pageSize, Int.ONE)))
    brkAddr = aligned
...
function Syscall.handle(cpu: CPU.CPU)
    sysno = Int.toNumber(cpu:readX(8))
    x0 = cpu:readX(0)
    x1 = cpu:readX(1)
    x2 = cpu:readX(2)
    x3 = cpu:readX(3)
    ...
    if sysno == 64 then # write
        ...
        function writeField(off: number, s: string)
            for idx = 1, s.count do
...
function Syscall.sysWrite(cpu: CPU.CPU, fdI: integer, bufAddr: integer, count: integer): integer
    fd = Int.toNumber(fdI)
    n = Int.toNumber(count)
    if fd == 1 or fd == 2 then
        data = cpu.mem:readString(bufAddr, n)
...
function Syscall._writeToConsole(data: string, fd: number)
    ...
    while true do
        nl = string.find(outputBuf[fd], "\n", 1, true)
        if not nl then break end
        line = string.sub(outputBuf[fd], 1, nl - 1)
        outputLine(line)
...
function Syscall.sysWritev(cpu: CPU.CPU, fdI: integer, iovAddr: integer, iovcnt: integer): integer
    fd = Int.toNumber(fdI)
    cnt = Int.toNumber(iovcnt)
    totalWritten = 0

    for idx = 0, cnt - 1 do
        iovBase = Int.add(iovAddr, Int.from(idx * 16))
        bufAddr = cpu.mem:readU64(iovBase)
        bufLen = Int.toNumber(cpu.mem:readU64(Int.add(iovBase, Int.from(8))))
        if bufLen > 0 then
            written = Syscall.sysWrite(cpu, fdI, bufAddr, Int.from(bufLen))
...
function Syscall.sysBrk(cpu: CPU.CPU, addr: integer): integer
    ...
    if Int.uge(addr, brkAddr) then
        ...
        oldBrk = brkAddr
        newBrk = Int.band(Int.add(addr, Int.from(4095)), Int.bnot(Int.from(4095)))
        diff = Int.toNumber(Int.sub(newBrk, oldBrk))
        if diff > 0 then
